...
class CStripTableModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self):
        ...
        # self.rowsRemoved.connect(self._rows_removed)
    ...
    def data(self, f_index, f_role=QtCore.Qt.DisplayRole):
        ...
        if not f_index.isValid() or not (0 <= f_index.row() < len(self._lst_strips)):
            # return
            return QtCore.QVariant()

        # get strip
        l_anv = self._lst_strips[f_index.row()]
        assert l_anv
    ...
    def _data_changed(self, f_index_tl, f_index_br):
        """
    ...
    def headerData(self, f_section, f_orientation, f_role=QtCore.Qt.DisplayRole):
        ...
        if f_orientation == QtCore.Qt.Horizontal:
            ...
            if f_section == ldefs.D_STP_ID:
                # return
                return QtCore.QVariant("ICAO ID")
    ...
    def insertRows(self, f_row, f_count=1, f_parent=QtCore.QModelIndex()):
        ...
        for l_row in xrange(f_count):
            ...
            self._lst_strips.insert(f_row + l_row, anvmdl.CAircraftVisil({}))

            # posiciona o strip !!!REVER!!!
            self._lst_strips[f_row + l_row].point = QtCore.QPointF(gdata.G_CTR_LAT, gdata.G_CTR_LNG)
    ...
    def removeRows(self, f_row, f_count=1, f_parent=QtCore.QModelIndex()):
        ...
        # rebuild strips list
        self._lst_strips = (self._lst_strips[:f_row] + self._lst_strips[f_row + f_count:])
    ...
    def rowCount(self, f_parent=QtCore.QModelIndex()):
        ...
        """
        # return
        return len(self._lst_strips)

    # ---------------------------------------------------------------------------------------------
    '''
    @QtCore.pyqtSlot(QtCore.QModelIndex, int, int)
    def _rows_removed(self, f_parent, fi_start=0, fi_end=0):
        """
    ...
    def setData(self, f_index, f_value, f_role=QtCore.Qt.EditRole):
        ...
        """
        if f_index.isValid() and (0 <= f_index.row() < len(self._lst_strips)):
            # get anv/strip
            l_anv = self._lst_strips[f_index.row()]
