...
class CFixData(dict):
    ...
    def make_fix(self, fdct_root, fdct_data):
        ...
        if "fixos" != fdct_root["tagName"]:
            ...
            l_log.critical("<E01: não é um arquivo de fixo.")
        ...
        if "nFix" in fdct_data:
            # cria fixo
            l_fix = model.CFixNEW(self._model, fdct_data, fdct_root["VERSION"])
            assert l_fix
        ...
        # senão, não existe indicativo
            ...
            # monta a mensagem
            ls_msg = "não tem identificação. Fixo não incluído."
    ...
    def parse_fix_xml(self, fs_fix_pn):
        ...
        if not l_data_file.isOpen():
            ...
            l_log.setLevel(logging.CRITICAL)
            l_log.critical("<E01: erro na abertura de {}.".format(fs_fix_pn))
        ...
        if not l_xdoc_fix.setContent(l_data_file):
            ...
            l_log = logging.getLogger("CFixData::parse_fix_xml")
            l_log.setLevel(logging.CRITICAL)
            l_log.critical("<E02: falha no parse de {}.".format(fs_fix_pn))
