...
class CAerNEW(model.CAerModel):
    ...
    def __init__(self, f_model, f_data=None, fs_ver="0001"):
        ...
        self._f_aer_dcl_mag = 0.

        # dicionário de pistas
        self._dct_aer_pistas = {}
        '''
        # meteorologia
    ...
    def calc_cab_oposta(self):
        ...
        for lp_pst in list(self._dct_aer_pistas.values()):
            # pista não ok ?
            ...
            for lp_pst_op in list(self._dct_aer_pistas.values()):
                # é a pista procurada ?
                if lp_pst_op.s_pst_indc.strip() == ls_oposta.strip():
                    ...
                    lf_lat, lf_lng, _ = self._model.coords.xyz2geo(lp_pst.f_pst_x, lp_pst.f_pst_y, 0.)
    ...
    def copy_aer(self, f_aer):
        ...
        # latitude do ARP (gr)
        self._f_aer_lat = f_aer.f_aer_lat
        # longitude do ARP (gr)
        self._f_aer_lng = f_aer.f_aer_lng
                                
        # declinação magnética (gr)
        self._f_aer_dcl_mag = f_aer.f_aer_dcl_mag
    ...
    def make_aer(self, fdct_data):
        ...
        if "nAer" in fdct_data:
            self.s_aer_indc = str(fdct_data["nAer"]).strip().upper()

        # descrição (nome)
        if "descricao" in fdct_data:
            self.s_aer_desc = str(fdct_data["descricao"]).strip()

        # declinação magnética (gr)
        if "declmag" in fdct_data:
            # declinação magnética em graus
            self._f_aer_dcl_mag = float(fdct_data["declmag"])
        ...
        if "coord" in fdct_data:
            self._f_aer_lat, self._f_aer_lng = self._model.coords.from_dict(fdct_data["coord"])
        ...
        if "pistas" in fdct_data:
            ...
            for l_pst in fdct_data["pistas"]:
                ...
                if li_pst is not None:
                    # cria pista
                    self._dct_aer_pistas[li_pst] = pstnew.CPstNEW(self._model, self, l_pst)
                    assert self._dct_aer_pistas[li_pst]

                    # ajusta rumo magnético e verdadeiro
                    self._dct_aer_pistas[li_pst].f_pst_true = self._dct_aer_pistas[li_pst].f_pst_rumo + self._f_aer_dcl_mag

                    # normaliza o rumo
                    if self._dct_aer_pistas[li_pst].f_pst_true < 0.:   
                        self._dct_aer_pistas[li_pst].f_pst_true += 360. 

                    elif self._dct_aer_pistas[li_pst].f_pst_true > 360.:
                        self._dct_aer_pistas[li_pst].f_pst_true -= 360.
